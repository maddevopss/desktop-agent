# ChronoMAD Desktop Agent

Agent Electron qui capture l'activite locale de l'utilisateur et l'envoie au backend ChronoMAD.

## Prerequis

- Node.js compatible avec Electron 42
- Backend ChronoMAD demarre et accessible
...
## Spotlight (décharge mentale)

Un raccourci global ouvre une barre flottante permettant de capturer une idée sans
quitter l'application en cours, puis de la retrouver dans l'inbox du Dashboard.

- Raccourci: `Ctrl+Shift+Espace`, avec repli automatique sur `Ctrl+Alt+Espace` si une
  autre application détient déjà la combinaison. Le raccourci retenu est affiché dans
  le menu du tray.
- `Entrée` capture et referme immédiatement; `Échap` ou la perte de focus annule.
- L'idée part sur `POST /api/brain-dump-captures`. En cas d'absence de token ou d'échec
  réseau, elle est mise dans la file offline persistante et rejouée plus tard. Les
  captures de décharge mentale sont exemptées de l'éviction FIFO de cette file: une
  capture d'activité est ré-échantillonnée en continu, une idée ne l'est pas.
- Aucun appel IA à l'écriture. Le découpage en micro-actions est déclenché
  explicitement depuis le Dashboard.
...
La signature Windows n'est pas configuree par defaut (`certificateFile: null`).
