const { shouldIgnoreActivity, getActivitySignature, isPrivacySensitiveTitle } = require("../src/utils/trackingFilter");
...
describe("trackingFilter", () => {
  describe("shouldIgnoreActivity", () => {
    ...
    test("applique les exclusions personnalisées", () => {
      expect(
        shouldIgnoreActivity(
          { app_name: "Microsoft Outlook", window_title: "Boîte de réception" },
          { ignoredApps: ["outlook"] },
        ),
      ).toBe(true);
      expect(
        shouldIgnoreActivity(
          { app_name: "Chrome", window_title: "Dossier médical du client" },
          { ignoredKeywords: ["médical"] },
        ),
      ).toBe(true);
    });
