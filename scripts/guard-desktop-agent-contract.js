...
const ipcPayloadSchemasPath = path.join(repoRoot, "src", "shared", "ipcPayloadSchemas.js");
...
const ipcPayloadSchemas = read(ipcPayloadSchemasPath);
...
if (!ipcPayloadSchemas) violations.push("src/shared/ipcPayloadSchemas.js is missing.");
...
if (ipc && !/ipcMain\.handle\("get-stored-token",\s*\(\)\s*=>\s*(?:null|\{\s*return null;\s*\})\s*\);/.test(ipc)) {
...
if (ipcPayloadSchemas && !ipcPayloadSchemas.includes("MAX_BRAIN_DUMP_LENGTH = 5_000")) {
  violations.push("brain dump schema must keep the 5000-character cap.");
}

if (ipc && !ipc.includes("const content = parseBrainDump(text);")) {
  violations.push("brain dump IPC must validate text through parseBrainDump before sending.");
}
